(function($) {
    $(function() {
    	var $ = jQuery,
    	serverPath = basePath+'filemanager/upload',
        $list = $('#thelist'),
        $btn = $('#ctlBtn'),
        state = 'pending',
        uploader=null,
        	...
        	if(md5!=null){
        		...
        		$('#form1').attr('action',serverPath).ajaxSubmit(function(msg){
        			...
        			if(  1 == msg.exist){
						...
						formData["appName"] = $('#appName').val();
						formData["appPkgName"] = $('#appPkgName').val();
						formData["version"] = $('#version').val();
						formData["district"] = $('select[name="district"]').val();
